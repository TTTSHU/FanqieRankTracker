...
| 🧭 类型风向标 | 独立趋势页聚合多日数据，用 AI 总结都市修真等综合赛道、具体热门分类和高频题材；未配置 API 时自动规则兜底 |
...
看板右上角的 **风向标** 可进入 `trend.html`，先查看当下火热综合赛道（如都市修真）、具体热门分类和高频题材，再按具体类型查看近 7 / 14 / 30 日或全部周期的趋势分析。全站热点会优先使用 AI 总结，未配置 API 或生成失败时使用规则统计文案兜底。
